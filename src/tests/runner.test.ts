...
import type { Implementation, TestCase, BenchmarkConfig } from '../types/index.js';

describe('BenchmarkRunner', () => {
  const runner = new BenchmarkRunner();

  const createImplementation = (code: string, entryFunction: string): Implementation => ({
    id: 'test-impl',
  ...
  describe('execute', () => {
    ...
    it('should handle errors gracefully', async () => {
      const impl = createImplementation(
        'function broken(x) { throw new Error("Test error"); }',
        'broken'
      );
    ...
    it('should collect memory metrics', async () => {
      const impl = createImplementation(
        'function alloc(n) { return new Array(n).fill(0); }',
        'alloc'
      );
